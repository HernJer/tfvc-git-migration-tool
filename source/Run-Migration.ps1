...
if (-not (Test-Path $resolvedConfig)) {
    ...
    Write-Host '  Run Configure.ps1 first, or supply -ConfigPath.' -ForegroundColor Yellow
    exit 1
}
...
if (-not $SkipExport -and -not $failed) {
    ...
    $ok = Invoke-Step -Number 1 -Name 'Export' -Script 'Export.ps1' -ExtraArgs $exportArgs
...
if (-not $DryRun -and -not $SkipReplay -and -not $failed) {
    ...
    $ok = Invoke-Step -Number 2 -Name 'Replay' -Script 'Replay.ps1' -ExtraArgs $replayArgs
    if (-not $ok) { $failed = $true }
}
...
if (-not $DryRun -and -not $SkipVerify -and -not $failed) {
    $ok = Invoke-Step -Number 3 -Name 'Verify' -Script 'Verify.ps1'
    if (-not $ok) { $failed = $true }
}
...
if (-not $DryRun -and -not $SkipReport -and -not $failed) {
    $ok = Invoke-Step -Number 4 -Name 'Report' -Script 'Report.ps1'
    if (-not $ok) { $failed = $true }
}
